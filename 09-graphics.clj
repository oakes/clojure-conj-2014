(defscreen graphics-screen
  :on-show
  (fn [screen entities]
    (update! screen :renderer (stage) :camera (orthographic))
    (let [ui-skin (skin "skin/uiskin.json")
          small-font (skin! ui-skin :get-font "small-font")
          small-style (style :label small-font (color :white))]
      (table [(stack [(image "images/lone-survivor.jpg"
                             :set-scaling (scaling :fit))
                      (label "Lone Survivor (2012)" small-style
                             :set-alignment (align :bottom-left))])
              (stack [(image "images/shovel-knight.jpg"
                             :set-scaling (scaling :fit))
                      (label "Shovel Knight (2014)" small-style
                             :set-alignment (align :bottom-left))])]
             :set-fill-parent true)))
  
  :on-render
  (fn [screen entities]
    (render! screen entities))
  
  :on-resize
  (fn [screen entities]
    (height! screen (:height screen))))
